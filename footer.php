<footer class="d-flex flex-column flex-md-row bg-dark text-light py-3 align-items-center">
    <div class="flex-fill text-center text-xxl-right py-3 py-md-0">
      <a href="http://www.github.com"><i class="fab fa-github"></i></a>
      <a href="http://www.facebook.com"><i class="fab fa-facebook-f"></i></a>
      <a href="http://www.linkedin.com"><i class="fab fa-linkedin"></i></a>
    </div>
    <div class="flex-fill text-center py-3 py-md-0">
        <h5>TRAVEL ON</h5>
    </div>
    <div class="flex-fill text-center text-xxl-left pb-2 pb-md-0">
        <img src="sources/images/caffeine-structure-light.png" alt="Travel Blog" width="130px">
    </div>
</footer>
